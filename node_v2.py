...
def expand_box(x1, y1, x2, y2, W, H, pad_ratio=0.03):
    """Expand a box by pad_ratio of its size, clamped to image extents."""
    w = max(1, x2 - x1)
    h = max(1, y2 - y1)
    px = int(w * pad_ratio)
    py = int(h * pad_ratio)
...
def get_control_points_balanced(mask_u8, total_k, min_sep=12, min_dt=0.5, min_area=1):
    """
    Ensure at least one point per connected component (largest first),
    then fill remaining with global DT peaks while enforcing min separation.
    mask_u8: 0/255 uint8 mask in ROI coords.
    Returns: list of [x, y] (float) in ROI coords.
    """
...
def hue_range_mask(hue_img_0_179, hmin_deg, hmax_deg):
    """
    Build a hue mask for OpenCV hue (0..179) given degree bounds (0..360).
    Handles wrap-around automatically.
    """
    hmin = int(round((hmin_deg % 360) / 2.0))
    hmax = int(round((hmax_deg % 360) / 2.0))
    if hmin <= hmax:
        return cv2.inRange(hue_img_0_179, hmin, hmax)
    else:
        # wrap-around: [hmin..179] U [0..hmax]
        m1 = cv2.inRange(hue_img_0_179, hmin, 179)
        m2 = cv2.inRange(hue_img_0_179, 0, hmax)
...
class GroundingDinoSAM2SegmentV2:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "max_bounding_boxes": ("INT", {"default": 3, "min": 1, "max": 10, "step": 1}),
                "enable_blob_detection": (["true", "false"], {"default": "true"}),

                # --- Positive (dummy color) thresholds ---
                ...
                # --- Negative thresholds (derived complement or manual) ---
                "auto_negative_from_pos": (["true", "false"], {"default": "true"}),
                "gap_hue": ("FLOAT", {"default": 10.0, "min": 0.0, "max": 90.0, "step": 1.0}),
                ...
                "num_negative_points": ("INT", {"default": 2, "min": 0, "max": 20, "step": 1}),
    ...
    def main(self, grounding_dino_model, sam_model, image, prompt, threshold,
             max_bounding_boxes, enable_blob_detection,
             pos_hue_min, pos_hue_max, pos_sat_min, pos_val_min,
        ...
        for item in image:
            ...
            # --- Positive (dummy) mask global ---
            pos_h_mask = hue_range_mask(Hch, pos_hue_min, pos_hue_max)
            pos_s_mask = (Sch >= int(round(pos_sat_min * 255))).astype(np.uint8) * 255
            pos_v_mask = (Vch >= int(round(pos_val_min * 255))).astype(np.uint8) * 255
            pos_mask_full = cv2.bitwise_and(pos_h_mask, cv2.bitwise_and(pos_s_mask, pos_v_mask))
            pos_mask_full = cv2.erode(pos_mask_full, kernel, iterations=1)
            pos_mask_full = cv2.dilate(pos_mask_full, kernel, iterations=1)

            # --- Negative mask global ---
            if auto_negative_from_pos == "true":
                # hue outside (pos band shrunk by gap_hue)
                inner_min = (pos_hue_min + gap_hue) % 360.0
                inner_max = (pos_hue_max - gap_hue) % 360.0
                hue_outside = cv2.bitwise_not(hue_range_mask(Hch, inner_min, inner_max))

                # low S or low V relative to pos mins minus gaps
                ns_max = max(0.0, float(pos_sat_min) - float(gap_sat))
                nv_max = max(0.0, float(pos_val_min) - float(gap_val))
                neg_s_mask = (Sch <= int(round(ns_max * 255))).astype(np.uint8) * 255
                neg_v_mask = (Vch <= int(round(nv_max * 255))).astype(np.uint8) * 255

                neg_mask_full = cv2.bitwise_or(hue_outside, cv2.bitwise_or(neg_s_mask, neg_v_mask))
            else:
                neg_h_mask = hue_range_mask(Hch, neg_hue_min, neg_hue_max)
                neg_s_mask = (Sch <= int(round(neg_sat_max * 255))).astype(np.uint8) * 255
                neg_v_mask = (Vch <= int(round(neg_val_max * 255))).astype(np.uint8) * 255
                # user-provided negatives: (hue in range) OR (low S) OR (low V)
                neg_mask_full = cv2.bitwise_or(neg_h_mask, cv2.bitwise_or(neg_s_mask, neg_v_mask))
            ...
            per_box_masks = []
            Himg, Wimg = image_np_rgb.shape[:2]

            union_bool_masks = []

            for bi in range(boxes_np.shape[0]):
                x1, y1, x2, y2 = [int(v) for v in boxes_np[bi]]
                x1, y1, x2, y2 = expand_box(x1, y1, x2, y2, Wimg, Himg, pad_ratio=0.03)
                if x2 <= x1 or y2 <= y1:
                    continue

                # ROI masks
                pm = pos_mask_full[y1:y2, x1:x2]
                nm = neg_mask_full[y1:y2, x1:x2]
                ...
                if enable_blob_detection == "true":
                    if num_positive_points > 0:
                        ...
                        pts_roi = get_control_points_balanced(
                            pm, total_k=num_positive_points,
                            min_sep=max(12, 3*erosion_kernel), min_dt=0.3, min_area=min_blob_px
                        )
                        pos_pts_abs = [[x1 + float(px), y1 + float(py)] for (px, py) in pts_roi]

                    if num_negative_points > 0:
                        # ring around positive region to keep negatives near boundary
                        dil = cv2.dilate(pm, kernel, iterations=1)
                        ring = cv2.subtract(dil, pm)
                        # intersect with color-based negs
                        ring_and_neg = cv2.bitwise_and(ring, nm)
                        neg_pts_roi = get_control_points(
                            ring_and_neg, min_size=3, max_num=num_negative_points,
                            min_sep=max(8, 2*erosion_kernel), min_dt=0.0
                        )
                        # fallback: use nm directly if ring too thin
                        if len(neg_pts_roi) < num_negative_points:
                            more = get_control_points(
                                nm, min_size=3, max_num=(num_negative_points - len(neg_pts_roi)),
                                min_sep=max(8, 2*erosion_kernel), min_dt=0.0
                            )
                            neg_pts_roi += more
                ...
                pl_i = np.array([1] * len(pos_pts_abs) + [0] * len(neg_pts_abs), dtype=np.int64) if all_pts else None

                # Draw only used points
                ...
                # Predict with SAM (multi-candidate)
                ...
                # Pick best by IoU with pos mask in ROI
                best = None
                best_iou = -1.0
                roi_ref = (pm > 0)
                for k in range(masks_i.shape[0]):
                    mk = masks_i[k].astype(bool)
                    iou = iou_bool(mk[y1:y2, x1:x2], roi_ref)
                    if iou > best_iou:
                        best_iou = iou
                        best = mk
                if best is None:
                ...
                # Coverage boost: add more positives if coverage poor
                ref_bool = (pm > 0)
                covered = np.logical_and(best[y1:y2, x1:x2], ref_bool).sum()
                total_ref = ref_bool.sum()
                coverage = float(covered) / float(total_ref + 1e-6)
                if enable_blob_detection == "true" and coverage < 0.80 and num_positive_points >= 4:
                    uncovered = cv2.bitwise_and(ref_bool.astype(np.uint8) * 255, cv2.bitwise_not(best[y1:y2, x1:x2].astype(np.uint8)) * 255)
                    ...
                    if extra_pts_roi:
                        ...
                        pc2 = np.concatenate([pc_i, np.array(extra_abs, dtype=np.float32)], axis=0) if pc_i is not None else np.array(extra_abs, dtype=np.float32)
                        pl2 = np.concatenate([pl_i, np.ones(len(extra_pts_roi), dtype=np.int64)], axis=0) if pl_i is not None else np.ones(len(extra_pts_roi), dtype=np.int64)
                        masks2, scores2, _ = predictor.predict(
                            point_coords=pc2, point_labels=pl2,
                            box=np.array([[x1, y1, x2, y2]], dtype=np.float32),
                            multimask_output=True
                        )
                        masks2 = _ensure_3d_mask(masks2)
                        for k in range(masks2.shape[0]):
                            mk2 = masks2[k].astype(bool)
                            iou2 = iou_bool(mk2[y1:y2, x1:x2], ref_bool)
                            if iou2 > best_iou:
                                best_iou = iou2
                                best = mk2
            ...
            # Build outputs (union across boxes)
            if len(union_bool_masks) > 0:
                final_bool = union_masks(union_bool_masks)
                images, masks = create_tensor_output(np.array(item), [final_bool[None, ...]], boxes)
